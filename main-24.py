...
def require_admin(authorization: str) -> str:
    """Verifies the request came from a logged-in admin. Returns the admin's email."""
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(401, "Missing Firebase auth token")
    id_token = authorization.split(" ", 1)[1]
    try:
...
ADMIN_EMAILS = {"engroscar516@gmail.com", "uzodinmachuks677@gmail.com"}


@app.get("/admin/stats")
async def admin_stats(authorization: str = Header(None)):
    require_admin(authorization)
    users_ref = db.collection("bpp_users")
    total_users = len(list(users_ref.stream()))
    vip_users = len(list(users_ref.where("vip", "==", True).stream()))
    return {"totalUsers": total_users, "vipUsers": vip_users}
